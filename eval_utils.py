...
def calculate_ssim(gt, pred, ssim):
    ...
    # TODO: This is a temporary fix for the black bars in the PEPPER dataset (using letterbox)
    # Remove lateral black bars (56 pixels)
    # gt = gt[:, :, :, :, 56:-56]
    # pred = pred[:, :, :, :, 56:-56]

    # normalize to [0, 1]
    # gt = gt.float().div(255) # before
    # pred = pred.float().div(255)
    gt = gt.float()
    pred = pred.float()
